...
fig1 = px.pie(values=pie_data.values(), names=pie_data.keys(), color=pie_data.keys(), labels=pie_data.keys(), title='Invested vs Returns', hole=0.4)
...
loan_n_int = emi_amt*tenure*12
interest_amt = round(loan_n_int-loan_amt)
t2col2.metric('Monthly EMI', round(emi_amt),)
t2col2.metric('Principal Amount', loan_amt,)
t2col2.metric('Total Interest', round(interest_amt),f'{round(interest_amt*100/loan_amt,2)}%')
t2col2.metric('Total Value', round(loan_n_int),f'{round(interest_amt/loan_amt,2)}x')
#?Tab2 pie chart
pie_data = {'Interest Amount': interest_amt, 'Principal Amount': loan_amt}
fig2 = px.pie(values=pie_data.values(), names=pie_data.keys(), color=pie_data.keys(), labels=pie_data.keys(), title='Principal vs Loan Interest', hole=0.4)
